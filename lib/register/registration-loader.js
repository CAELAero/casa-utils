...
export class CASARegistrationLoader {
     ...
     *   a zero length array
     */
    static listAllRegistrations(source) {
        if (!source) {
        ...
        sheet_data.forEach((row) => {
            try {
                ...
                const holder_add = Address.create2Line(CASALoaderUtils.parseString(row[13]), CASALoaderUtils.parseString(row[14]), CASALoaderUtils.parseString(row[15]), CASALoaderUtils.parseString(row[16]), holder_postcode, CASALoaderUtils.parseString(row[18]));
                ...
                const operator_add = Address.create2Line(CASALoaderUtils.parseString(row[21]), CASALoaderUtils.parseString(row[22]), CASALoaderUtils.parseString(row[23]), CASALoaderUtils.parseString(row[24]), operator_postcode, CASALoaderUtils.parseString(row[26]));
...
//# sourceMappingURL=registration-loader.js.map