 ...
 */
export class CASARegistrationChangeLoader {
    /**
     ...
     *   a zero length array
     */
    static listAllRegistrationChanges(source) {
        if (!source) {
        ...
        sheet_data.forEach((row) => {
            try {
                const entry = new RegistrationChangeData();
                entry.mark = 'VH-' + row[0];
                ...
                const holder_add = Address.create2Line(CASALoaderUtils.parseString(row[7]), CASALoaderUtils.parseString(row[8]), CASALoaderUtils.parseString(row[9]), CASALoaderUtils.parseString(row[10]), holder_postcode, CASALoaderUtils.parseString(row[12]));
                ...
                const operator_add = Address.create2Line(CASALoaderUtils.parseString(row[14]), CASALoaderUtils.parseString(row[15]), CASALoaderUtils.parseString(row[16]), CASALoaderUtils.parseString(row[17]), operator_postcode, CASALoaderUtils.parseString(row[19]));
...
//# sourceMappingURL=registration-change-loader.js.map